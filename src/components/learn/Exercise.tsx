...
function FillBlankExercise({
  ...
  const handleCheck = () => {
    onAttempt()
    const checked = answers.map((a, i) => a.trim().toUpperCase() === (blanks[i] ?? '').trim().toUpperCase())
    setResults(checked)
...
export function Exercise(props: ExerciseProps) {
  const {
    id, title, description, type, template, blanks,
    starterCode, database, schema, expectedQuery, expectedResult, strictOrder,
    options, correctIndex, hints, explanation, lang
  } = props
