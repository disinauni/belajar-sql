...
export function ProgressTracker({ lessonSlug, lang, totalLessons }: ProgressTrackerProps) {
  const [progress, setProgress] = useState<UserProgress | null>(null)
  const total = totalLessons ?? getTotalLessons()
  ...
  const overall = getOverallProgress(total)
...
function badgeLabel(badge: string, lang: Language): string {
  const labels: Record<string, Record<Language, string>> = {
    ...
    'unit-0-complete': { id: 'Unit 0 Selesai', en: 'Unit 0 Complete' },
    ...
    'first-playground': { id: 'Proyek Mini Pertama', en: 'First Mini Project' },
